...
import { X, ArrowDown, ArrowUp, CreditCard, Percent, LucideDollarSign, Repeat, AlertTriangle } from "lucide-react";
...
export default function AddModal({ cards, onClose, onSave, onSaveInstallment, allExpCats, allIncCats, onAddCat, editing, userId, installmentPlans }) {
  ...
  useEffect(() => {
    ...
    const defaults = { income: "fixed", expense: "loan", "card-purchase": "loan" };
  ...
  useEffect(() => {
    if (type !== "card-payment" || isEditing || amountUserEdited.current) return;
  ...
  useEffect(() => {
    if (!isEditing && userId && category && type !== "card-payment" && type !== "card-interest") {
  ...
  const handleSave = () => {
    if (isInstallment) {
      if (!amount || +amount <= 0 || !installLabel.trim() || +installMonths < 1 || !cardId) return;
      onSaveInstallment({
  ...
  const valid = isInstallment
    ? amount && +amount > 0 && installLabel.trim() && +installMonths >= 1 && cardId
    : amount && +amount > 0 && (!isCardType || cardId);

  // Installment limit warning
  const selectedCard = cards.find((c) => c.id === cardId);
  const planTotal = isInstallment && +amount > 0 && +installMonths > 0 ? +amount * +installMonths : 0;
  const availableCredit = selectedCard ? (+selectedCard.limit || 0) - selectedCard.currentBalance : 0;
  const exceedsLimit = isInstallment && planTotal > 0 && selectedCard?.limit && planTotal > availableCredit;
      ...
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        {isEditing && editing?.installmentId && (
          <div className="hint warn-hint">
            <AlertTriangle size={13} /> This transaction is part of an installment plan — editing it here won't update the plan's monthly amount.
          </div>
        )}
        ...
        {isInstallment && (
          <>
            ...
            <input type="text" className="text-input" value={installLabel}
              onChange={(e) => setInstallLabel(e.target.value)}
              placeholder="e.g. Samsung TV" autoFocus />
            <label className="field-lbl">Number of months</label>
            <input type="number" className="text-input" value={installMonths}
              onChange={(e) => setInstallMonths(e.target.value)}
              min="2" max="120" placeholder="12" />
            <label className="field-lbl">Start month</label>
            <input type="month" className="text-input" value={installStartMonth}
            ...
            {exceedsLimit && (
              <div className="hint warn-hint">
                <AlertTriangle size={13} /> Plan total ({CURRENCY} {fmtCompact(planTotal)}) exceeds available credit ({CURRENCY} {fmtCompact(availableCredit)}).
              </div>
            )}
          </>
        )}
        ...
        <button className={`save-btn ${!valid ? "disabled" : ""}`} onClick={handleSave} disabled={!valid}>
          {isInstallment ? "Create Plan" : isEditing ? "Save changes" : "Record"}
        </button>
      </div>
    </div>
  );
