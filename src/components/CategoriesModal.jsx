...
import { Edit2, Trash2, ArrowDown, ArrowUp, Plus } from "lucide-react";
import CategoryFormModal from "./CategoryFormModal.jsx";
import Sheet from "./Sheet.jsx";
import { getProtectedCategory } from "../constants/categories.js";
  ...
  return (
    <Sheet title="Categories" onClose={onClose}>
      ...
      <div className="manage-cat-list">
        {cats.map((c) => {
          const Icon = c.icon;
          const isProtected = !!getProtectedCategory(tab, c.id);
          return (
            <div key={c.id} className="manage-cat-row">
              ...
              <button
                ...
                title={isProtected ? "This category can't be deleted" : "Delete"}
                onClick={() => {
                  if (!isProtected && confirm(`Delete the "${c.label}" category?`))
                    onDelete(tab, c.id);
                }}
                aria-label="Delete"
              >
