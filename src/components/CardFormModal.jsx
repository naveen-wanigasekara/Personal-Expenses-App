...
export default function CardFormModal({ card, onClose, onSave }) {
  ...
  const [displayLimit, setDisplayLimit] = useState(() =>
    limit ? fmtLive(String(limit)) : "",
  );
  const [displayOpeningBalance, setDisplayOpeningBalance] = useState(() =>
    openingBalance ? fmtLive(String(openingBalance)) : "",
  );
  const [colorIdx, setColorIdx] = useState(
    card?.colors
      ? CARD_COLORS.findIndex((c) => c[0] === card.colors[0])
    ...
    <div className="backdrop" onClick={onClose}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <input
          ...
          placeholder="e.g. HSBC Visa, Sampath Platinum"
          autoFocus
        />
        ...
        <div className="big-input">
          ...
          <input
            ...
            onChange={(e) =>
              numericChange(e, setOpeningBalance, setDisplayOpeningBalance)
            }
            placeholder="0.00"
            inputMode="decimal"
          />
        ...
        <div className="hint">
          {card
            ? "Editing this adjusts the card's starting balance. Transactions still count on top."
